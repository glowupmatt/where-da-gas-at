...
function SavedSpotsComponent() {
  ...
  const onDeleteHandler = async (id) => { 
    try {
      ...
      const updatedStations = selectedStations.filter((station) => station.id !== id);
      ...
      try {
        ...
        const stationsData = await Promise.all(
          userStations.map(async (spot) => {
            const selectedStationData = await fetchSelectedStation(spot.id);
            const photoUrls = await fetchPhotoUrl(selectedStationData);
    ...
    return (
      <div className="saved-spots-container">
        ...
        <div className="confirmation-buttons">
          <button className="button" onClick={() => onDeleteHandler(openConfirmation)}>Yes</button>
          <button className="button" onClick={() => setOpenConfirmation(null)}>No</button>
        </div>
      </div>
    )
  }
        ...
        if (spot) {
          return (
            <div key={spot + index} className="saved-spot-item">
              <div className="delete-icon-container" onClick={() => setOpenConfirmation(spot.id)}>
                <FaTrash className="delete-icon" />
              </div>
              <DisplayCard selectedStation={spot} photoUrl={photoUrl[spot.id]} />
            </div>
          )
        }
    })}
