...
const handleSave = async () => {
  ...
  try {
    const res = await fetch(`/api/king/current/station/${selectedStation.id}`);
    console.log(res);
    ...
    console.error("Failed to check station existence:", err);
  }

  try {
    const response = await fetch(`/api/king/current/station/${String(selectedStation.id)}`, {
      method: "POST",
      headers: {
    ...
    setMessage(`Station saved successfully: ${selectedStation.displayName.text}`);
  } catch (err) {
