...
def test_update_user():
    ...
    user_slice = state["user"]
    assert len(user_slice) == 1
    update_user = next(iter(user_slice.values()))
    required_keys = ["email", "id", "user"]

    assert all(key in update_user for key in required_keys)
    assert all(key in required_keys for key in update_user)
    ...
    assert updated_user["email"] == "a" + user["email"]
    assert updated_user["user"] == "a" + user["user"]

    # tests/api/test_user_routes.py ..update_user_response = <Response [200]>
    # updated_user = {'user': {'19': {'email': 'aanrdcjmxfn@example.com', 'id': 19, 'user': 'ablklfffoln'}}}
